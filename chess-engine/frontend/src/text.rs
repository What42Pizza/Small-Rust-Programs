...
pub fn render_text<'a>(text: impl AsRef<str>, size: f32, x: i32, mut y: i32, foreground: Color, background: Color, canvas: &mut Canvas<Window>, texture_creator: &'a TextureCreator<WindowContext>, text_cache: &mut TextCache<'a>, font: &impl ThreadSafeFont) -> Result<()> {
	...
	for texture_data in new_textures.into_inner().unwrap() {
		...
		text_cache.map.insert((c, (size * 100.0) as usize, foreground, background), (texture, x_offset, y_offset));
	}
	...
	if let Some((c, glyph)) = glyphs.first() {
		...
		if let Some((texture, x_offset, y_offset)) = texture_data {
	...
	for [(_prev_c, prev_glyph), (c, glyph)] in glyphs.array_windows() {
		...
		x += font.h_advance(glyph.id);
		x += size * 0.03;
		if c.is_whitespace() {x += size * 0.03;}
	}
	Ok(())
}
...
pub fn ensure_glyph_is_rasterized(glyph: Glyph, c: char, foreground: Color, background: Color, font: &PxScaleFont<&impl ThreadSafeFont>) -> Result<Option<(char, Vec<u8>, u32, u32, f32, f32)>> {
	
	let Some(glyph) = font.outline_glyph(glyph) else {return Ok(None);};
	...
	Ok(Some((c, pixels, width, height, -bounds.min.x / 3.0, -bounds.min.y)))
}
