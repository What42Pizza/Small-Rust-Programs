...
impl<T> GuiElement<T> {
	...
	pub fn child(&self, name: &str) -> &GuiElement<T> {
		if let Some(slash_index) = name.find('/') {
			let child = match self.children_by_name.get(&name[..slash_index]) {
				Some(&i) => &self.children_by_layer[i],
				None => panic!("Could not find gui element \"{}\" in \"{}\"", &name[..slash_index], self.name),
			};
			return child.child(&name[slash_index+1..]);
		}
		match self.children_by_name.get(name) {
			Some(&i) => &self.children_by_layer[i],
			None => panic!("Could not find gui element \"{name}\" in \"{}\"", self.name),
		}
	}
	
	pub fn child_mut(&mut self, name: &str) -> &mut GuiElement<T> {
		if let Some(slash_index) = name.find('/') {
			let child = match self.children_by_name.get(&name[..slash_index]) {
				Some(&i) => &mut self.children_by_layer[i],
				None => panic!("Could not find gui element \"{}\" in \"{}\"", &name[..slash_index], self.name),
			};
			return child.child_mut(&name[slash_index+1..]);
		}
		match self.children_by_name.get(name) {
			Some(&i) => &mut self.children_by_layer[i],
			None => panic!("Could not find gui element \"{name}\" in \"{}\"", self.name),
	...
	pub fn try_child(&self, name: &str) -> Option<&GuiElement<T>> {
		self.children_by_name
			.get(name)
			.map(|&index| &self.children_by_layer[index])
	}
	
	pub fn try_child_mut(&mut self, name: &str) -> Option<&mut GuiElement<T>> {
		self.children_by_name
			.get(name)
			.map(|&index| &mut self.children_by_layer[index])
	}
