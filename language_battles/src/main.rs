...
fn main() -> Result<()> {
	loop {
		if !prompt!("Calculate results? "; [true] YesNoInput) {return Ok(());}
		calculate_totals()?;
	}
}
...
fn calculate_totals() -> Result<()> {
	
	// get input
	let mut input_path = fns::get_program_dir();
	input_path.push("input.txt");
	let input = fs::read_to_string(&input_path).with_context(|| format!("while trying to read {input_path:?}"))?;
